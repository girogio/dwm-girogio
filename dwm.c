...
/* Enable if you want to control gaps*/
...
void drawbar(Monitor *m)
{
    ...
    if (m == selmon)
    {
        ...
        drw_text(drw, m->ww - tw, 0, tw, bh, lrpad / 2, 0, stext, 0);
    }
    ...
    for (i = 0; i < LENGTH(tags); i++)
    {
        ...
        case 0:
            drw_text(drw, x, 0, w, bh, (lrpad - 1) / 2, upad, tags[i], urg & 1 << i);
            break;
        case 1:
    ...
    drw_setscheme(drw, scheme[SchemeNorm]);
    //Layout indicator
    x = drw_text(drw, x, 0, w, bh, lrpad / 2, 0, m->ltsymbol, 0);
    ...
    if ((w = m->ww - tw - x) > bh)
    {
        if (m->sel)
        {
            ...
            drw_text(drw, x, 0, w, bh, mid, 0, m->sel->name, 0);
            if (m->sel->isfloating)
...
void resource_load(XrmDatabase db, char *name, enum resource_type rtype, void *dst)
{
    ...
    if (!(ret.addr == NULL || strncmp("String", type, 64)))
    {
        ...
        case ARR_INTEGER:
            ...
            while (token)
            {
                *aidst[pos] = strtoul(token, NULL, 10);
                pos++;
                token = strtok(NULL, ",");
            }
            break;
